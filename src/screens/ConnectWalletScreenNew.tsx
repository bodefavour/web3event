import { SafeAreaView, StyleSheet, View, Pressable, ScrollView, ActivityIndicator, Alert } from 'react-native';
...
export const ConnectWalletScreen = ({ onBack, onSelectWallet }: Props) => {
  ...
  const handleWalletConnect = async (walletId: string, walletType: string) => {
    ...
    try {
      if (walletType === 'hedera') {
        ...
        // Handle EVM wallets via ThirdWeb
        // ThirdWeb's ConnectWallet component will handle this
        Alert.alert(
          'Connect Wallet',
          `Please use the "Connect Wallet" button below to connect your ${walletOptions.find(w => w.id === walletId)?.label}`,
          [{ text: 'OK' }]
        );
      }
    } catch (error: any) {
  ...
  const handleHederaWalletConnect = async (walletId: string) => {
    try {
      // For Hedera wallets, we'll use deep linking
      if (walletId === 'hashpack') {
        // HashPack pairing
        const pairingString = await createHashPackPairing();
        Alert.alert(
          'HashPack Pairing',
          'Opening HashPack wallet...',
          [
            {
              text: 'Cancel',
              style: 'cancel'
        ...
        // Deep link to HashPack
        // Linking.openURL(`hashpack://pairing?data=${pairingString}`);
        
      } else if (walletId === 'blade') {
        // Blade wallet connection
        Alert.alert(
          'Blade Wallet',
          'Opening Blade wallet...',
          [
            {
              text: 'Cancel',
              style: 'cancel'
        ...
        // Deep link to Blade
        // Linking.openURL(`blade://connect`);
      }

      // For demo purposes, simulate successful connection
      setTimeout(() => {
        const mockHederaAccount = '0.0.12345';
        handleWalletConnected(mockHederaAccount, walletId);
      }, 2000);
  ...
  const handleWalletConnected = async (walletAddress: string, walletType: string) => {
    try {
      ...
      Alert.alert(
        'Wallet Connected',
        `Successfully connected ${walletType}`,
        [
          {
            text: 'Continue',
            onPress: () => onSelectWallet(walletType)
          }
        ]
      );
    } catch (error: any) {
      ...
      >
        ...
        <View style={styles.section}>
          <ThemedText style={[styles.helperText, { color: palette.textSecondary, marginBottom: spacing.md }]}>
            Or use ThirdWeb's wallet connection:
          </ThemedText>
          <ConnectWallet
            theme="dark"
